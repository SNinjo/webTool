...
function setKeyEvent(e){
    ...
    if (e.key === "ArrowUp"){
        ...
        if ((lastKey != "ArrowUp") && (lastKey != "ArrowDown")) {
            vOffset = window.getSelection().focusOffset;
        }
...
function getCursor(){
    ...
    var objFocused = window.getSelection().focusNode; // selected paragraph

    var objParagraph = objFocused.parentNode;
    var intNestedNumber = 1;
    while (objParagraph.id.indexOf("paragraph") != -1){
        if (objParagraph == document) return null;

        objParagraph = objFocused.parentNode;
        intNestedNumber++;
    }
    ...
    var arraySiblings = objFocused.childNodes;
    for (var index = 0; index < arraySiblings.length; index++){
        var element = arraySiblings[index];
        if (element == objFocused) break;

        /*console.log(element)//
        console.log(objFocused)//
        console.log(element == objFocused)//*/
